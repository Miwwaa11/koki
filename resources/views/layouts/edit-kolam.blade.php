        ...
        <header class="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 px-5 pt-12 pb-6 rounded-b-3xl shadow-soft relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex justify-between items-center mb-4">
                    <button onclick="window.location.href='{{ route('kolam.show', \->id) }}'" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-all btn-press">
        ...
        <main class="px-5 py-6 pb-24 space-y-5">
            ...
            <div class="glass-card rounded-2xl p-5 shadow-card">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="font-bold text-slate-800">{{ $kolam->nama_kolam }}</h2>
                        <p class="text-sm text-slate-500">ID: {{ $kolam->id }} • {{ $kolam->totalIkan() }}/{{ $kolam->kapasitas }} ekor</p>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Edit Kolam -->
            <div class="glass-card rounded-2xl p-5 shadow-card">
                <form action="{{ route('kolam.update', $kolam->id) }}" method="POST" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_kolam" value="{{ old('nama_kolam', $kolam->nama_kolam) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white transition-colors @error('nama_kolam') border-red-500 @enderror" required>
                        @error('nama_kolam')
                    ...
                    <div>
                        ...
                        <input type="number" name="kapasitas" value="{{ old('kapasitas', $kolam->kapasitas) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white @error('kapasitas') border-red-500 @enderror" required>
                        @error('kapasitas')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="ukuran" value="{{ old('ukuran', $kolam->ukuran) }}" placeholder="Contoh: 3x4m, 12m²" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white transition-colors">
                    ...
                    <div>
                        ...
                        <input type="text" name="lokasi" value="{{ old('lokasi', $kolam->lokasi) }}" placeholder="Contoh: Area A, Gedung 1" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white transition-colors">
                        @error('lokasi')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="suhu" value="{{ old('suhu', $kolam->suhu) }}" placeholder="26" step="0.01" min="0" max="100" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white transition-colors @error('suhu') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            <label class="flex items-center gap-2">
                                <input type="radio" name="status" value="aktif" class="w-4 h-4 text-emerald-600" {{ old('status', $kolam->status) == 'aktif' ? 'checked' : '' }} required>
                                <span class="text-sm">Aktif</span>
                            </label>
                            <label class="flex items-center gap-2">
                                <input type="radio" name="status" value="nonaktif" class="w-4 h-4 text-emerald-600" {{ old('status', $kolam->status) == 'nonaktif' ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-3">
                        ...
                        <button type="button" onclick="window.location.href='{{ route('kolam.show', $kolam->id) }}'" class="py-3 rounded-xl bg-slate-100 text-slate-600 font-semibold hover:bg-slate-200 transition-all btn-press">
                ...
                <div class="max-w-3xl mx-auto">
                    ...
                    <div class="glass-card rounded-2xl p-8 shadow-card">
                        <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-xl mb-6">
                            ...
                            <div>
                                <h2 class="font-bold text-slate-800 text-xl">{{ $kolam->nama_kolam }}</h2>
                                <p class="text-sm text-slate-500">ID: {{ $kolam->id }} • {{ $kolam->totalIkan() }} ekor / {{ $kolam->kapasitas }} ekor</p>
                            </div>
                        </div>

                        <form action="{{ route('kolam.update', $kolam->id) }}" method="POST" class="space-y-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="nama_kolam" value="{{ old('nama_kolam', $kolam->nama_kolam) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white @error('nama_kolam') border-red-500 @enderror" required>
                                    @error('nama_kolam')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="ukuran" value="{{ old('ukuran', $kolam->ukuran) }}" placeholder="Contoh: 3x4m" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white">
                                    @error('ukuran')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="lokasi" value="{{ old('lokasi', $kolam->lokasi) }}" placeholder="Contoh: Area A" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/70 focus:bg-white">
                                    @error('lokasi')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="bg-emerald-50 p-4 rounded-xl">
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="suhu" value="{{ old('suhu', $kolam->suhu) }}" step="0.01" min="0" max="100" class="w-full px-4 py-2 rounded-lg border border-emerald-200 bg-white focus:bg-white @error('suhu') border-red-500 @enderror">
                                        @error('suhu')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                            ...
                            <div class="flex gap-3 pt-4">
                                ...
                                <button type="button" onclick="window.location.href='{{ route('kolam.show', $kolam->id) }}'" class="px-8 py-3 rounded-xl bg-slate-100 text-slate-600 font-semibold hover:bg-slate-200 transition-all btn-press">
