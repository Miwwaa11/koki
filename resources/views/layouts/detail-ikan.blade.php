...
<head>
    ...
    <style>
        ...
        .dark .wave-pattern { background-image: none !important; }
    ...
    <div class="md:hidden max-w-[414px] mx-auto min-h-screen bg-gradient-to-b from-slate-50 to-white relative shadow-2xl overflow-hidden">
        ...
        <header class="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 px-5 pt-12 pb-6 rounded-b-3xl shadow-soft relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex justify-between items-center mb-4">
                    <a href="{{ route('ikan.index') }}" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-all btn-press">
        ...
        <main class="px-5 py-6 pb-24 space-y-6 hide-scrollbar overflow-y-auto" style="height: calc(100vh - 100px);">
                ...
                <div class="glass-card rounded-2xl p-6 shadow-card">
                    <div class="flex flex-col items-center">
                        ...
                        <div class="relative mb-4">
                            ...
                            <span class="absolute -top-2 -right-2 px-3 py-1 rounded-full text-[10px] font-bold {{ $ikan->status_kesehatan == 'Stabil' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : ($ikan->status_kesehatan == 'Observasi' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-red-100 text-red-700 border border-red-200') }} shadow-md">
                                <i class="fas fa-{{ $ikan->status_kesehatan == 'Stabil' ? 'check-circle' : ($ikan->status_kesehatan == 'Observasi' ? 'exclamation-circle' : 'triangle-exclamation') }} mr-1"></i>{{ $ikan->status_kesehatan }}
                            </span>
                        </div>

                        <!-- Nama Ikan -->
                        <h2 class="text-2xl font-bold text-slate-800 mb-1">{{ $ikan->jenis_ikan }}</h2>

                        <!-- Jenis & Ukuran -->
                        <div class="flex items-center gap-3 text-sm text-slate-500 mb-4">
                            <span class="flex items-center gap-1"><i class="fas fa-tag text-emerald-500"></i> {{ $ikan->jenis_ikan }}</span>
                            @if($ikan->ukuran)
                            <span class="w-1 h-1 bg-slate-300 rounded-full"></span>
                            <span class="flex items-center gap-1"><i class="fas fa-ruler text-emerald-500"></i> {{ $ikan->ukuran }}</span>
                            @endif
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 w-full mt-2">
                            <div class="text-center p-3 bg-slate-50 rounded-xl">
                                ...
                                <p class="font-bold text-emerald-600">Rp {{ number_format($ikan->harga, 0, ',', '.') }}</p>
                            </div>
                            <div class="text-center p-3 bg-slate-50 rounded-xl">
                                <p class="text-xs text-slate-500">Stok</p>
                                <p class="font-bold text-slate-800">{{ number_format($ikan->stok) }}</p>
                            </div>
                            <div class="text-center p-3 bg-slate-50 rounded-xl">
                                <p class="text-xs text-slate-500">Ukuran</p>
                                <p class="font-bold text-slate-800">{{ $ikan->ukuran ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="animate-slide-up" style="animation-delay: 0.15s;">
                ...
                <div class="glass-card rounded-2xl p-4 shadow-card card-hover">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1">
                            <h3 class="font-bold text-slate-800">{{ $ikan->kolam->nama_kolam }}</h3>
                            <p class="text-xs text-slate-500 mb-1">Kapasitas {{ number_format($ikan->kolam->kapasitas) }} ekor</p>
                            <div class="flex items-center gap-2">
                                <span class="text-xs {{ $ikan->kolam->status == 'aktif' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600' }} px-2 py-0.5 rounded-full">{{ ucfirst($ikan->kolam->status) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section class="animate-slide-up" style="animation-delay: 0.25s;">
                ...
                <!-- Tombol Pindah Kolam -->
                <a href="{{ route('pindah-ikan') }}" class="w-full py-3 rounded-xl bg-gradient-to-r from-teal-500 to-emerald-600 text-white font-semibold shadow-md shadow-teal-500/30 hover:shadow-lg hover:scale-[1.02] transition-all btn-press flex items-center justify-center gap-2">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">{{ number_format($ikan->stok) }} ekor</p>
                                </div>
                            </div>
                            <span class="text-xs text-slate-400">{{ $ikan->created_at ? $ikan->created_at->diffForHumans() : '-' }}</span>
                        </div>
                    </div>
                </div>
            </section>
                            ...
                            <div class="glass-card rounded-2xl p-6 shadow-card sticky top-24">
                                <div class="flex flex-col items-center">
                                    <div class="relative mb-4">
                                        ...
                                        <span class="absolute -top-2 -right-2 px-3 py-1 rounded-full text-xs font-bold {{ $ikan->status_kesehatan == 'Stabil' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : ($ikan->status_kesehatan == 'Observasi' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-red-100 text-red-700 border border-red-200') }} shadow-md">
                                            {{ $ikan->status_kesehatan }}
                                        </span>
                                    </div>
                                    <h2 class="text-2xl font-bold text-slate-800">{{ $ikan->jenis_ikan }}</h2>
                                    <p class="text-emerald-600 font-medium mt-1">{{ $ikan->jenis_ikan }}{{ $ikan->ukuran ? ' • ' . $ikan->ukuran : '' }}</p>
                                    ...
                                    <div class="w-full grid grid-cols-3 gap-2 mt-4">
                                        ...
                                        <div class="text-center p-2 bg-slate-50 rounded-lg">
                                            <p class="text-xs text-slate-500">Stok</p>
                                            <p class="font-bold">{{ number_format($ikan->stok) }}</p>
                                        </div>
                                        <div class="text-center p-2 bg-slate-50 rounded-lg">
                                            <p class="text-xs text-slate-500">Ukuran</p>
                                            <p class="font-bold">{{ $ikan->ukuran ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="md:col-span-2 space-y-6">
                            ...
                            <div class="glass-card rounded-2xl p-6 shadow-card">
                                ...
                                <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                                    ...
                                    <div class="flex-1">
                                        <h3 class="font-bold text-slate-800 text-lg">{{ $ikan->kolam->nama_kolam }}</h3>
                                        <p class="text-sm text-slate-500 mb-2">Kapasitas {{ number_format($ikan->kolam->kapasitas) }} ekor</p>
                                        <div class="flex items-center gap-3">
                                            <span class="text-xs {{ $ikan->kolam->status == 'aktif' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600' }} px-2 py-1 rounded-full">{{ ucfirst($ikan->kolam->status) }}</span>
                                        </div>
                                    </div>
                                </div>
                                @else
                            ...
                            <div class="space-y-3">
                                <!-- Tombol Pindah Kolam -->
                                <a href="{{ route('pindah-ikan') }}" class="w-full py-3 rounded-xl bg-gradient-to-r from-teal-500 to-emerald-600 text-white font-semibold shadow-md shadow-teal-500/30 hover:shadow-lg hover:scale-[1.02] transition-all btn-press text-center block">
                                    ...
                                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-slate-500">{{ number_format($ikan->stok) }} ekor</p>
                                            </div>
                                        </div>
                                        <span class="text-xs text-slate-400">{{ $ikan->created_at ? $ikan->created_at->diffForHumans() : '-' }}</span>
                                    </div>
