...
<head>
    ...
    <style>
        ...
        /* Modal Adjustment */
        .dark #groupDetailModal .bg-white {
            background-color: #1a1d23 !important;
            color: white;
        }

        .dark #groupDetailModal .sticky {
            background-color: #1a1d23 !important;
        ...
        .fish-card-enter:nth-child(1) {
            animation-delay: 0.1s;
        }

        .fish-card-enter:nth-child(2) {
            animation-delay: 0.15s;
        }

        .fish-card-enter:nth-child(3) {
            animation-delay: 0.2s;
        }

        .fish-card-enter:nth-child(4) {
            animation-delay: 0.25s;
        }

        .fish-card-enter:nth-child(5) {
            animation-delay: 0.3s;
        }

        .fish-card-enter:nth-child(6) {
...
<body class="text-slate-800 antialiased wave-pattern">
        ...
        <header class="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 px-5 pt-12 pb-6 rounded-b-3xl shadow-soft relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex justify-between items-center mb-4">
                    <button onclick="window.location.href='{{ route('dashboard') }}'" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-all btn-press">
        ...
        <main class="px-5 py-6 pb-24 space-y-6 hide-scrollbar overflow-y-auto" style="height: calc(100vh - 100px);">
            ...
            <section class="animate-slide-up" style="animation-delay: 0.1s;">
                <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-2" id="filterChips">
                    ...
                    <button class="filter-chip px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-600 text-xs font-semibold hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-all whitespace-nowrap btn-press" data-filter="{{ $jenis }}" onclick="filterByChip(this, '{{ $jenis }}')">
                        {{ $jenis }}
                    </button>
                    @endforeach
                </div>
            ...
            <section class="animate-slide-up" style="animation-delay: 0.2s;">
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <div class="glass-card rounded-2xl p-3 shadow-card border-l-4 border-emerald-500 card-hover stat-card-enter btn-press cursor-pointer">
                        ...
                        <p class="text-xl font-extrabold gradient-text">{{ $ikans->sum('stok') }}</p>
                    ...
                    <div class="glass-card rounded-2xl p-3 shadow-card border-l-4 border-teal-500 card-hover stat-card-enter btn-press cursor-pointer">
                        ...
                        <p class="text-xl font-extrabold gradient-text">{{ $ikans->pluck('jenis_ikan')->unique()->count() }}</p>
                    ...
                    <div class="glass-card rounded-2xl p-3 shadow-card border-l-4 border-cyan-500 card-hover stat-card-enter btn-press cursor-pointer">
                        ...
                        <p class="text-xl font-extrabold gradient-text">{{ $ikans->where('status_kesehatan', 'sehat')->sum('stok') }}</p>
                        <p class="text-[8px] text-slate-400 mt-0.5">ekor</p>
                    </div>
                </div>
            </section>
            ...
            <section class="animate-slide-up" style="animation-delay: 0.3s;">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-[10px] text-slate-400 font-medium bg-slate-100 px-2 py-1 rounded-md">{{ $ikans->count() }} Item</span>
                </div>

                <div class="space-y-3">

                    @forelse($flattenedGroupedIkan as $group)
                    <!-- Fish Card Group {{ $loop->iteration }} -->
                    <div class="glass-card rounded-2xl p-4 shadow-card card-hover btn-press fish-card-enter"
                        data-ikan-item
                        data-jenis="{{ $group['jenis_ikan'] }}"
                        data-ukuran="{{ $group['ukuran'] ?? '' }}"
                        data-kolam="{{ implode(',', $group['kolam_names']->toArray() ?? []) }}">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-start mb-1">
                                    <div>
                                        <h3 class="font-bold text-slate-800 text-base truncate">{{ $group['jenis_ikan'] }}</h3>
                                        @if($group['ukuran'])
                                        <p class="text-[10px] text-slate-500 mt-0.5">Ukuran: {{ $group['ukuran'] }}</p>
                                        @endif
                                    </div>
                                    <span class="px-2.5 py-1 rounded-full text-[9px] font-bold {{ $group['status_kesehatan'] == 'Stabil' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : ($group['status_kesehatan'] == 'Observasi' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 'bg-red-100 text-red-700 border border-red-200') }}">
                                        <i class="fas fa-{{ $group['status_kesehatan'] == 'Stabil' ? 'check-circle' : ($group['status_kesehatan'] == 'Observasi' ? 'exclamation-circle' : 'triangle-exclamation') }} mr-1"></i>{{ $group['status_kesehatan'] }}
                                    </span>
                                </div>
                                <div class="flex items-center gap-2 text-xs text-slate-500 mb-2">
                                    <span class="flex items-center gap-1"><i class="fas fa-tag text-emerald-500"></i> {{ $group['jenis_ikan'] }}</span>
                                </div>
                                @if(count($group['kolam_names']) > 0)
                                <div class="flex flex-wrap gap-1.5 mb-2">
                                    @foreach($group['kolam_names'] as $kolamName)
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-medium bg-blue-50 text-blue-700 border border-blue-200">
                                        <i class="fas fa-water text-[8px]"></i> {{ $kolamName }}
                                    </span>
                                    @endforeach
                                </div>
                                @else
                                ...
                                <div class="flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-bold text-emerald-600">{{ number_format($group['total_stok']) }} ekor</p>
                                        @if($group['jumlah_entry'] > 1)
                                        <p class="text-[9px] text-slate-400">{{ $group['jumlah_entry'] }} batch</p>
                                        @endif
                                    </div>
                                    <p class="text-[10px] text-slate-400">Rp {{ number_format($group['harga'], 0, ',', '.') }}/ekor</p>
                                </div>
                            </div>
                        </div>
                        <div class="flex gap-2 mt-4 pt-3 border-t border-slate-100">
                            @if($group['jumlah_entry'] == 1)
                            <a href="{{ route('ikan.show', $group['ikan_details'][0]['id']) }}" class="flex-1 py-2 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                <i class="fas fa-eye mr-1"></i> Detail
                            </a>
                            @else
                            <button onclick="showGroupDetails('{{ $group['jenis_ikan'] }}', '{{ $group['ukuran'] }}')" class="flex-1 py-2 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                <i class="fas fa-layer-group mr-1"></i> Lihat Detail ({{ $group['jumlah_entry'] }})
                            </button>
                            @endif
                        </div>
                    </div>
                ...
                <div class="max-w-7xl mx-auto space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-6">
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-emerald-500 card-hover">
                            ...
                            <p class="text-3xl font-extrabold gradient-text">{{ $ikans->sum('stok') }}</p>
                        ...
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-teal-500 card-hover">
                            ...
                            <p class="text-3xl font-extrabold gradient-text">{{ $ikans->pluck('jenis_ikan')->unique()->count() }}</p>
                        ...
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-cyan-500 card-hover">
                            ...
                            <p class="text-3xl font-extrabold gradient-text">{{ $ikans->where('status_kesehatan', 'sehat')->sum('stok') }}</p>
                    ...
                    <div class="glass-card rounded-2xl p-6 shadow-card">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <span class="text-sm text-slate-400 font-medium bg-slate-100 px-3 py-1.5 rounded-lg">{{ $ikans->count() }} Item</span>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @forelse($flattenedGroupedIkan as $group)
                            <!-- Fish Card Group Desktop {{ $loop->iteration }} -->
                            <div class="glass-card rounded-2xl p-5 shadow-card card-hover btn-press border border-slate-100"
                                data-ikan-item-desktop
                                data-jenis-desktop="{{ $group['jenis_ikan'] }}"
                                data-ukuran-desktop="{{ $group['ukuran'] ?? '' }}"
                                data-kolam-desktop="{{ implode(',', $group['kolam_names']->toArray() ?? []) }}">
                                <div class="flex items-start gap-4 mb-4">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                <h3 class="font-bold text-slate-800">{{ $group['jenis_ikan'] }}</h3>
                                                @if($group['ukuran'])
                                                <p class="text-[10px] text-slate-500 mt-0.5">Ukuran: {{ $group['ukuran'] }}</p>
                                                @endif
                                            </div>
                                        ...
                                        <p class="text-xs text-slate-500">
                                            <i class="fas fa-tag text-emerald-500 mr-1"></i> {{ $group['jenis_ikan'] }}
                                        </p>
                                        @if(count($group['kolam_names']) > 0)
                                ...
                                <div class="flex items-center justify-between mb-4">
                                    <div>
                                        <p class="text-lg font-bold text-emerald-600">{{ number_format($group['total_stok']) }} ekor</p>
                                        @if($group['jumlah_entry'] > 1)
                                        <p class="text-[9px] text-slate-400">{{ $group['jumlah_entry'] }} batch</p>
                                        @endif
                                    </div>
                                    <p class="text-xs text-slate-400">Rp {{ number_format($group['harga'], 0, ',', '.') }}/ekor</p>
                                </div>
                                <div class="flex gap-2">
                                    @if($group['jumlah_entry'] == 1)
                                    <a href="{{ route('ikan.show', $group['ikan_details'][0]['id']) }}" class="flex-1 py-2.5 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                        <i class="fas fa-eye mr-1"></i> Detail
                                    </a>
                                    @else
                                    <button onclick="showGroupDetailsDesktop('{{ $group['jenis_ikan'] }}', '{{ $group['ukuran'] }}')" class="flex-1 py-2.5 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                        <i class="fas fa-layer-group mr-1"></i> Lihat Detail ({{ $group['jumlah_entry'] }})
                                    </button>
                                    @endif
                                </div>
    ...
    <script>
        ...
        const groupedIkanData = @json($flattenedGroupedIkan);
