...
<body class="text-slate-800 antialiased wave-pattern">
    ...
    <div class="md:hidden max-w-[414px] mx-auto min-h-screen bg-gradient-to-b from-slate-50 to-white relative shadow-2xl overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <div class="flex justify-between items-center mb-5">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('dashboard') }}" class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white hover:bg-white/30 transition-all btn-press">
                            <i class="fas fa-arrow-left text-sm"></i>
                        </a>
                        <img src="{{ asset('images/alfin2.png') }}" alt="Logo" class="h-8 w-auto">
                    </div>
        ...
        <main class="px-5 py-6 pb-24 space-y-6 hide-scrollbar overflow-y-auto" style="height: calc(100vh - 100px);">
            ...
            <section class="animate-slide-up" style="animation-delay: 0.1s;">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-card border-l-4 border-emerald-500 card-hover stat-card-enter btn-press cursor-pointer">
                        ...
                        <p class="text-2xl font-extrabold gradient-text">{{ $jumlahKolam }}</p>
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-card border-l-4 border-teal-500 card-hover stat-card-enter btn-press cursor-pointer">
                        ...
                        <p class="text-2xl font-extrabold gradient-text">{{ $kolamAktif }}</p>
                        <p class="text-[8px] text-slate-400 mt-1">unit</p>
                    </div>
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-card border-l-4 border-cyan-500 card-hover stat-card-enter btn-press cursor-pointer">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            @php
                                $totalKapasitas = $kolams->sum('kapasitas');
                                $totalIkanDiKolam = $kolams->sum('total_ikan');
                                $percentageKapasitas = $totalKapasitas > 0 ? round(($totalIkanDiKolam / $totalKapasitas) * 100) : 0;
                            @endphp
                            <span class="text-[10px] font-semibold text-cyan-600 bg-cyan-50 px-2 py-1 rounded-full">{{ $percentageKapasitas }}%</span>
                        </div>
                        <p class="text-[9px] text-slate-500 font-medium uppercase tracking-wider mb-2">Kapasitas Kolam</p>
                        <div class="space-y-1.5">
                            <div class="flex justify-between items-center">
                                <span class="text-[8px] text-slate-600">Total Ikan:</span>
                                <span class="text-sm font-bold text-slate-800">{{ number_format($totalIkanDiKolam) }} ekor</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-[8px] text-slate-600">Kapasitas:</span>
                                <span class="text-sm font-bold text-slate-800">{{ number_format($totalKapasitas) }} ekor</span>
                            </div>
                            <div class="flex justify-between items-center pt-1 border-t border-cyan-100">
                                <span class="text-[8px] text-slate-600 font-medium">Tersedia:</span>
                                <span class="text-sm font-bold text-emerald-600">{{ number_format($totalKapasitas - $totalIkanDiKolam) }} ekor</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card rounded-2xl p-4 shadow-card border-l-4 border-amber-500 card-hover stat-card-enter btn-press cursor-pointer">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            @php
                                $kolamBukanAktif = $kolams->where('status', '!=', 'aktif')->count();
                            @endphp
                            <span class="text-[10px] font-semibold text-amber-600 bg-amber-50 px-2 py-1 rounded-full">{{ $kolamBukanAktif }}</span>
                        </div>
                        <p class="text-[9px] text-slate-500 font-medium uppercase tracking-wider mb-1">Perlu Perhatian</p>
                        <p class="text-2xl font-extrabold gradient-text">{{ $kolamBukanAktif }}</p>
            ...
            <section class="animate-slide-up" style="animation-delay: 0.2s;">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-[10px] text-slate-400 font-medium bg-slate-100 px-2 py-1 rounded-md">{{ $kolamAktif }} Aktif</span>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @forelse($kolams as $kolam)
                        @php
                            $statusBadgeClass = match($kolam->status) {
                                'aktif' => 'bg-emerald-100 text-emerald-700 border-emerald-200',
                                default => 'bg-slate-200 text-slate-600 border-slate-300'
                            };
                            $borderColor = match($kolam->status) {
                                'aktif' => 'border-emerald-500',
                                default => 'border-slate-300'
                            };
                            $progressColor = match($kolam->status) {
                                'aktif' => 'bg-emerald-500',
                                default => 'bg-slate-300'
                            };
                            $totalIkan = $kolam->total_ikan ?? 0;
                            $percentageUsed = $kolam->kapasitas > 0 ? round(($totalIkan / $kolam->kapasitas) * 100) : 0;
                        @endphp
                    
                        <div class="glass-card rounded-2xl p-4 shadow-card card-hover btn-press pond-card-enter border-l-4 {{ $borderColor }} @if($kolam->status !== 'aktif') opacity-80 @endif">
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    <h3 class="font-bold text-slate-800 text-base">{{ $kolam->nama_kolam }}</h3>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="px-2.5 py-1 rounded-full text-[8px] font-bold {{ $statusBadgeClass }} border flex items-center gap-1">
                                            @if($kolam->status === 'aktif')
                                                <i class="fas fa-circle text-[6px]"></i> Aktif
                                            @else
                                                <i class="fas fa-tools text-[8px]"></i> {{ ucfirst($kolam->status) }}
                                            @endif
                                        </span>
                                        <span class="text-[9px] text-slate-500 flex items-center gap-1">
                                            <i class="fas fa-fish text-emerald-500"></i> {{ $totalIkan }} ekor
                                        </span>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <span class="text-[9px] text-slate-500 block">Kapasitas</span>
                                    <span class="text-xs font-bold text-slate-800">{{ $totalIkan }}/{{ $kolam->kapasitas }}</span>
                                </div>
                            </div>
                            
                            <div class="flex items-center gap-3 text-[10px] text-slate-500 mb-4">
                                <span class="flex items-center gap-1"><i class="fas fa-tag text-emerald-500"></i> {{ $kolam->ukuran ?? 'N/A' }}</span>
                                <span class="w-1 h-1 bg-slate-300 rounded-full"></span>
                                <span class="flex items-center gap-1"><i class="fas fa-temperature-high text-emerald-500"></i> {{ $kolam->suhu ?? '-' }}°C</span>
                                <span class="w-1 h-1 bg-slate-300 rounded-full"></span>
                                <span class="flex items-center gap-1"><i class="fas fa-droplet text-emerald-500"></i> 7.2 pH</span>
                            </div>
                            
                            <!-- Progress Bar -->
                            <div class="w-full h-1.5 bg-slate-100 rounded-full mb-4">
                                <div class="h-1.5 {{ $progressColor }} rounded-full w-[{{ $percentageUsed }}%]"></div>
                            </div>
                            
                            <!-- Actions -->
                            <div class="flex gap-2">
                                <a href="{{ route('kolam.show', $kolam->id) }}" class="flex-1 py-2.5 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                    <i class="fas fa-eye mr-1"></i> Lihat Detail
                                </a>
                                <a href="{{ route('kolam.edit', $kolam->id) }}" class="flex-1 py-2.5 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-teal-50 hover:text-teal-600 transition-colors btn-press text-center">
                                    <i class="fas fa-edit mr-1"></i> Edit
                                </a>
                            </div>
                        </div>
                    @empty
                        <div class="glass-card rounded-2xl p-8 shadow-card text-center">
                            <i class="fas fa-inbox text-4xl text-slate-300 mb-3"></i>
                            <p class="text-slate-500 font-medium">Tidak ada data kolam</p>
                        </div>
                    @endforelse
        ...
        <a href="{{ route('kolam.create') }}" class="fixed bottom-24 right-6 z-30 group btn-press mobile-only">
            <div class="absolute inset-0 bg-emerald-500 rounded-full blur opacity-40 group-hover:opacity-60 transition-opacity animate-pulse-slow"></div>
            <div class="relative w-14 h-14 rounded-full bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 text-white shadow-lg shadow-emerald-500/40 flex items-center justify-center text-xl hover:scale-110 hover:shadow-floating hover:rotate-90 transition-all duration-300">
                <i class="fas fa-plus"></i>
            </div>
            <span class="absolute right-16 top-1/2 -translate-y-1/2 bg-slate-800 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                Tambah Kolam
            </span>
        </a>
        ...
        <nav class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-200 px-6 py-3 pb-6 z-40 max-w-[414px] mx-auto shadow-soft">
            <div class="flex justify-around items-center">
                <a href="{{ route('dashboard') }}" class="flex flex-col items-center gap-1.5 nav-item text-slate-400 hover:text-emerald-600 transition-all duration-300 btn-press">
                ...
                <a href="{{ route('kolam.index') }}" class="flex flex-col items-center gap-1.5 nav-item nav-item-active transition-all duration-300 btn-press">
                    <div class="nav-icon w-10 h-10 rounded-full flex items-center justify-center">
                        <i class="fas fa-water text-base"></i>
                    </div>
                    <span class="text-[9px] font-semibold">Kolam</span>
                </a>
         ...
         <!-- ==================== SIDEBAR ==================== -->
            <aside class="sidebar-width bg-white border-r border-slate-200 flex flex-col fixed h-full shadow-elegant z-50">
                ...
                <div class="px-5 py-4 border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('dashboard') }}" class="w-9 h-9 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-600 hover:bg-emerald-200 hover:text-emerald-700 transition-colors flex-shrink-0">
                            <i class="fas fa-arrow-left text-sm"></i>
                        </a>
                ...
                <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-slate-600 hover:bg-slate-50 transition-all sidebar-link">
                    ...
                    <a href="{{ route('kolam.index') }}" class="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 text-emerald-700 font-semibold transition-all sidebar-link active">
            ...
            <header class="px-8 py-5">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('kolam.create') }}" class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold shadow-lg shadow-emerald-500/30 hover:shadow-elegant hover:scale-105 transition-all btn-press">
                            <i class="fas fa-plus mr-2"></i>Tambah Kolam
                        </a>
                    </div>
                </div>
            </header>
                ...
                <div class="max-w-7xl mx-auto space-y-6">
                    ...
                    <div class="grid grid-cols-4 gap-6">
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-emerald-500 card-hover">
                            ...
                            <p class="text-3xl font-extrabold gradient-text">{{ $jumlahKolam }}</p>
                        ...
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-teal-500 card-hover">
                            ...
                            <p class="text-3xl font-extrabold gradient-text">{{ $kolamAktif }}</p>
                            <p class="text-[10px] text-slate-400 mt-1">unit</p>
                        </div>
                        ...
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-cyan-500 card-hover">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                @php
                                    $totalKapasitasDesk = $kolams->sum('kapasitas');
                                    $totalIkanDiKolamDesk = $kolams->sum('total_ikan');
                                    $percentageKapasitasDesk = $totalKapasitasDesk > 0 ? round(($totalIkanDiKolamDesk / $totalKapasitasDesk) * 100) : 0;
                                @endphp
                                <span class="text-xs font-semibold text-cyan-600 bg-cyan-50 px-2 py-1 rounded-full">{{ $percentageKapasitasDesk }}%</span>
                            </div>
                            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider mb-3">Kapasitas Kolam</p>
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-slate-600">Total Ikan:</span>
                                    <span class="text-lg font-bold text-slate-800">{{ number_format($totalIkanDiKolamDesk) }} ekor</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-slate-600">Kapasitas:</span>
                                    <span class="text-lg font-bold text-slate-800">{{ number_format($totalKapasitasDesk) }} ekor</span>
                                </div>
                                <div class="flex justify-between items-center pt-2 border-t border-cyan-100">
                                    <span class="text-xs text-slate-600 font-medium">Tersedia:</span>
                                    <span class="text-lg font-bold text-emerald-600">{{ number_format($totalKapasitasDesk - $totalIkanDiKolamDesk) }} ekor</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="glass-card rounded-2xl p-6 shadow-card border-l-4 border-amber-500 card-hover">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                @php
                                    $kolamBukanAktifDesk = $kolams->where('status', '!=', 'aktif')->count();
                                @endphp
                                <span class="text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-1 rounded-full">{{ $kolamBukanAktifDesk }}</span>
                            </div>
                            <p class="text-xs text-slate-500 font-medium uppercase tracking-wider mb-2">Perlu Perhatian</p>
                            <p class="text-3xl font-extrabold gradient-text">{{ $kolamBukanAktifDesk }}</p>
                    ...
                    <div class="glass-card rounded-2xl p-6 shadow-card">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <span class="text-sm text-slate-400 font-medium bg-slate-100 px-3 py-1.5 rounded-lg">{{ $kolamAktif }} Aktif</span>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @forelse($kolams as $kolam)
                                @php
                                    $statusBadgeClassDesk = match($kolam->status) {
                                        'aktif' => 'bg-emerald-100 text-emerald-700 border-emerald-200',
                                        default => 'bg-slate-200 text-slate-600 border-slate-300'
                                    };
                                    $borderColorDesk = match($kolam->status) {
                                        'aktif' => 'border-emerald-500',
                                        default => 'border-slate-300'
                                    };
                                    $progressColorDesk = match($kolam->status) {
                                        'aktif' => 'bg-emerald-500',
                                        default => 'bg-slate-300'
                                    };
                                    $totalIkanDesk = $kolam->total_ikan ?? 0;
                                    $percentageUsedDesk = $kolam->kapasitas > 0 ? round(($totalIkanDesk / $kolam->kapasitas) * 100) : 0;
                                @endphp
                                
                                <div class="glass-card rounded-2xl p-5 shadow-card card-hover btn-press border-l-4 {{ $borderColorDesk }}">
                                    <div class="flex items-start justify-between mb-3">
                                        <div>
                                            <h3 class="font-bold text-slate-800">{{ $kolam->nama_kolam }}</h3>
                                            <div class="flex items-center gap-2 mt-1">
                                                <span class="px-2.5 py-1 rounded-full text-[8px] font-bold {{ $statusBadgeClassDesk }} border">
                                                    @if($kolam->status === 'aktif')
                                                        Aktif
                                                    @else
                                        ...
                                        <div class="text-right">
                                            ...
                                            <p class="text-sm font-bold text-slate-800">{{ $totalIkanDesk }}/{{ $kolam->kapasitas }}</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3 text-xs text-slate-500 mb-3">
                                        <span><i class="fas fa-tag text-emerald-500 mr-1"></i>{{ $kolam->ukuran ?? 'N/A' }}</span>
                                        <span>•</span>
                                        <span><i class="fas fa-temperature-high text-emerald-500 mr-1"></i>{{ $kolam->suhu ?? '-' }}°C</span>
                                        <span>•</span>
                                        <span><i class="fas fa-droplet text-emerald-500 mr-1"></i>7.2 pH</span>
                                    </div>
                                    <div class="w-full h-1.5 bg-slate-100 rounded-full mb-4">
                                        <div class="h-1.5 {{ $progressColorDesk }} rounded-full w-[{{ $percentageUsedDesk }}%]"></div>
                                    </div>
                                    <div class="flex gap-2">
                                        <a href="{{ route('kolam.show', $kolam->id) }}" class="flex-1 py-2 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-600 transition-colors btn-press text-center">
                                            <i class="fas fa-eye mr-1"></i> Detail
                                        </a>
                                        <a href="{{ route('kolam.edit', $kolam->id) }}" class="flex-1 py-2 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-teal-50 hover:text-teal-600 transition-colors btn-press text-center">
                                            <i class="fas fa-edit mr-1"></i> Edit
                                        </a>
                                    </div>
                                </div>
                            @empty
                                <div class="col-span-2 glass-card rounded-2xl p-8 shadow-card text-center">
                                    <i class="fas fa-inbox text-4xl text-slate-300 mb-3"></i>
                                    <p class="text-slate-500 font-medium">Tidak ada data kolam</p>
                                </div>
                            @endforelse
    ...
    <a href="{{ route('kolam.create') }}" class="hidden md:flex fixed bottom-8 right-8 w-14 h-14 rounded-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-floating items-center justify-center text-xl z-50 btn-press hover:scale-110 transition-all duration-300">
        <i class="fas fa-plus"></i>
    </a>
